...
export class PV {
    ...
    static varString(movelist: Move[], maxlen: number = 4) {
        ...
        const movelist_string = movelist
            .map((move) => `[${move.source} -> ${move.target}]`)
            .slice(0, maxlen)
            .join(", ");
...
export class Engine {
    static i2r = new Map<number, number>([
        [0, 1],
        [1, 2],
        [2, 2],
        [3, 3],
    ...
    static score = (board: boolean[]): number =>
        14 ** 2 - board.reduce((acc, val) => acc + (val ? 1 : 0), 0) ** 2;
    ...
    static async search(board: boolean[], pv: PV) {
