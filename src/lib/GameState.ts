...
export class GameStateManager {
    ...
    isSelectable = (index: number) => {
        ...
        if (!filled && this.state.selected != -1) {
            ...
            for (const move of moves) {
                if (move.target == index && this.state.board[move.capture]) return true
            }
    ...
    handleClick = (index: number) => {
        ...
        if (this.isSelectable(index)) {
            if (index == this.state.selected) this.setState({ ...this.state, selected: -1 })
    ...
    makeMove = (index: number) => {
        ...
        for (const move of moves) {
            console.log(`makeMove: capture: ${move.capture}, target: ${move.target}`)
            if (move.target == index && this.state.board[move.capture]) {
                console.log(`makeMove: changing state for capture: ${move.capture}, target: ${move.target}`)
    ...
    getHint = () => {
        ...
        console.log(pv)
        if (pv.best_variations.length > 0) this.setState({ ...this.state, pv: pv.best_variations, selected: pv.best_variations[0].movelist[0].source })
